...
class Node:
    def __init__(self, char, length = None, data = None):
        ...
        self.data = None
        self.children = {}
        # length값을 정할 dictionary. 코드를 간소화하려고 defaultdict을 사용해 인자값이 없으면 0을 리턴하도록.
...
class Trie:
    ...
    def insert(self, string):
        ...
        for char in string:
            ...
            # 해당 노드를 거쳐가는 문자열 중 길이가 len(length)인 것의 개수 저장
    ...
    def start_with(self, prefix, length):
        ...
        # prefix의 마지막 노드에서 length변수를 확인해 해당 노드를 거쳐간 문자열 중 길이가 length인 것의 개수를 반환
